...
import {Button, CardMedia, Checkbox, FormControlLabel, Grid, Icon, Radio, RadioGroup} from "@material-ui/core";
...
export default function PaymentMethod() {
    ...
    useEffect(() => {
        ...
        if (rate && rate?.bpi) {
            ...
            if (filters.length > 0) {
                ...
                setRateValue((rate?.bpi[filters[0]].rate_float / rate?.bpi["USD"].rate_float) || 1);
            }
        }
        setChangeRate(rate);
    }, [methods, rate]);
    ...
    const handleRadioChanged = (event) => {
        const valuesSelected = paymentsMethod.filter((it) => it.id.indexOf(event?.target?.value) === 0);
    ...
    return (
        <React.Fragment>
            <RadioGroup aria-label="payment-type" name="paymentType"
                        value={paymentSelected?.id || null} onChange={handleRadioChanged}>
                        ...
                        paymentsMethod.map((payment, index) => {
                            return (
                                <Grid className="card" item key={index} xs={12} sm={6} md={4}>
                                    <FormControlLabel value={payment.id}
                    ...
                    <Grid item xs={12} sm={8} md={6}>
                        <p>
                                    <span style={{
                                        ...
                                        $ {(promotionSelected?.promotionalPrice ? promotionSelected?.promotionalPrice : promotionSelected?.basePrice).toFixed(2)}
                            ...
                            <Grid container justify="center">
                                ...
                                <Grid item xs={12} sm={8} md={6}>
                                    <span style={{
                                        ...
                                        {
                                            moment(new Date(changeRate.time.updatedISO)).format('DD/MM/YYYY HH:mm')
                                        }
                                    </span>
                                </Grid>
                            </Grid>
                        ...
                        paymentSelected?.paymentMethod?.code === 'PROXY_PAY_ANG' && (
                            <>
                                <Button
                                    ...
                                    endIcon={<Icon>monetization_on</Icon>}>
                                    Generar Referencia
                                </Button>
                            </>
                        )
